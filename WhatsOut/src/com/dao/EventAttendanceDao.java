...
public class EventAttendanceDao {
	 ...
	 * This function register information about attendance
	...
	public boolean insert(EventAttendance attendance) {
		...
		boolean result = qex.insert(query,attendance.getSignDate().toString(), attendance.getUser().getId(), attendance.getEvent().getId());
		qex.closeConnection();
		return result;
	}
	/*
	 * Written on March 20, 2018 
	 * This function deletes an attendance using the attendance id
	 * Uses the model written by Rupendre Maharjan
	 * Uses the QueryExecutor Class written by Yvan GAKUBA
	 * @Author Yvan GAKUBA
	 * */
	public boolean delete(int id) {
		QueryExecutor qex = new QueryExecutor();
		String query="DELETE FROM eventAttendance WHERE id=?";
	...
	/*
	 * Written on March 20, 2018 
	 * This function all events you want to attend and those you attended
	 * Uses the model written by Rupendre Maharjan
	 * Uses the QueryExecutor Class written by Yvan GAKUBA
	 * @Author Yvan GAKUBA
	 * */
	public List<EventAttendance> get(int userID) {
		...
		try {
			...
			while (rs.next()) {
				...
				eventsAttendance.add(new EventAttendance(rs.getInt(1), LocalDate.of(Integer.parseInt(date[0]),Integer.parseInt(date[1]),Integer.parseInt(date[2])), 
